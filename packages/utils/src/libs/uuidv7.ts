...
export class V7Generator {
  ...
  generateOrAbortWithTs(unixTsMs: number): UUID | undefined {
    ...
    if (!Number.isInteger(unixTsMs) || unixTsMs < 0 || unixTsMs > 0xffff_ffff_ffff) {
...
export const uuidv7obj = (): UUID => (defaultGenerator ??= new V7Generator()).generate();
...
export const uuidv4obj = (): UUID => (defaultGenerator ??= new V7Generator()).generateV4();
