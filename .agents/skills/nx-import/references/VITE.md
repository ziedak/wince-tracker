...
Vite-specific guidance for `nx import`. For generic import issues (bun globs, root deps, project references, name collisions, ESLint, frontend tsconfig base settings, `@nx/react` typings, Jest preset, non-Nx source handling), see `SKILL.md`.
...
Plugin init loads `vite.config.ts` before deps are available. **Fix**: `bun add -wD vite @vitejs/plugin-react` (or `@vitejs/plugin-vue`) first, then `bun exec nx add @nx/vite`.
...
React 18 (source) + React 19 (dest): bun may hoist mismatched `react-dom`, causing `TypeError: Cannot read properties of undefined (reading 'S')`. **Fix**: Align versions with `bun.overrides`.
...
1. `bun add -wD eslint@^9 eslint-plugin-vue vue-eslint-parser @vue/eslint-config-typescript @typescript-eslint/parser @nx/eslint-plugin typescript-eslint`
...
`vue-eslint-parser` must be an explicit bun dependency (strict resolution prevents transitive import).
...
1. Generic fixes from SKILL.md (bun globs, root deps, executor paths, frontend tsconfig base settings, `@nx/react` typings)
...
1. Generic fixes from SKILL.md (stale files cleanup, bun globs, rewritten scripts, target name prefixing, noEmit→composite, ESLint handling)
