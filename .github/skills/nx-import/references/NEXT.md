...
Next.js-specific guidance for `nx import`. For generic import issues (bun globs, root deps, project references, name collisions, ESLint, frontend tsconfig base settings, `@nx/react` typings, Jest preset, target name prefixing, non-Nx source handling), see `SKILL.md`.
...
**Fix**: Install deps at the root before building: `bun add -wD @types/react @types/react-dom`
...
2. Generic fixes from SKILL.md (bun globs, root deps, `.gitkeep` removal, frontend tsconfig base settings, `@nx/react` typings)
3. Install Next.js-specific deps: `bun add -wD @next/eslint-plugin-next`
...
2. Generic fixes from SKILL.md (bun globs, stale files cleanup, script rewriting, target name prefixing)
...
- Errors found & fixed:
  1. bun-workspace.yaml: `apps`/`libs` → `apps/*`/`libs/*`
...
- Errors found & fixed:
  1. bun-workspace.yaml: `apps/web` → `apps/*`
  2. Stale files: `node_modules/`, `bun-lock.yaml`, `bun-workspace.yaml`, `.gitignore` — deleted
...
- Next.js-specific findings:
  ...
  2. Stale files: `node_modules/`, `package-lock.json`, `.gitignore` — deleted (npm workspace, no bun files)
...
- Errors found & fixed:
  ...
  2. Stale files from Vite source: `node_modules/`, `bun-lock.yaml`, `bun-workspace.yaml`, `.gitignore`, `nx.json`
  3. Removed rewritten scripts from Vite app's `package.json`
  4. ESLint 8 vs 9 conflict — `@nx/eslint` peer on ESLint 8 resolved wrong version. Fixed with `bun.overrides`
